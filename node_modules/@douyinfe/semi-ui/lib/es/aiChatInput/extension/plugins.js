 ...
 */
export function handleZeroWidthCharLogic(newState) {
  let todoPositions = [];
  let {
    tr
  } = newState;
...
export function ensureTrailingText(schema) {
  return new Plugin({
...
export function keyDownHandlePlugin(schema) {
  return new Plugin({
    ...
    props: {
      handleKeyDown(view, event) {
        ...
        if (event.key === 'Backspace' && selection.empty) {
           ...
           */
          if ($from.nodeBefore && $from.nodeBefore.isText && ((_a = $from.nodeBefore.text) === null || _a === void 0 ? void 0 : _a.length) === 1 && $from.nodeBefore.text !== strings.ZERO_WIDTH_CHAR && $from.nodeAfter && $from.nodeAfter.attrs.isCustomSlot) {
        ...
        if (node.type.name === 'inputSlot') {
          ...
          if (node.textContent === strings.ZERO_WIDTH_CHAR && (event.key === 'ArrowLeft' || event.key === 'ArrowRight')) {
          ...
          if ($from.pos === $from.end() && node.textContent.length === 1 && node.textContent !== strings.ZERO_WIDTH_CHAR && event.key === 'Backspace') {
            const pos = $from.pos - 1;
          ...
          if (node.textContent === strings.ZERO_WIDTH_CHAR && event.key === 'Backspace') {
            // 计算当前节点在文档中的位置
...
export function handlePasteLogic(view, event) {
  const {
    state,
    dispatch
  } = view;
...
export function removeZeroWidthChar($from, tr) {
...
export function removeZeroWidthCharForComposition($from, tr) {
  // 检查光标左侧的 text node 是否以零宽字符开头
  if ($from.nodeBefore && $from.nodeBefore.isText) {
    const text = $from.nodeBefore.text;
    if (text === null || text === void 0 ? void 0 : text.startsWith(strings.ZERO_WIDTH_CHAR)) {
      // 删除第一个字符
...
export function handleCompositionEndLogic(view) {
  const {
    state,
    dispatch
  } = view;
...
export function handleTextInputLogic(view, from, to, text) {
  const {
    state,
    dispatch
  } = view;
