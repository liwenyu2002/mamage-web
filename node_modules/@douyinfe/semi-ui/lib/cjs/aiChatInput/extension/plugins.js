...
exports.ensureTrailingText = ensureTrailingText;
exports.handleCompositionEndLogic = handleCompositionEndLogic;
exports.handlePasteLogic = handlePasteLogic;
exports.handleTextInputLogic = handleTextInputLogic;
exports.handleZeroWidthCharLogic = handleZeroWidthCharLogic;
exports.keyDownHandlePlugin = keyDownHandlePlugin;
exports.removeZeroWidthChar = removeZeroWidthChar;
exports.removeZeroWidthCharForComposition = removeZeroWidthCharForComposition;
  ...
  newState.doc.descendants((node, pos, parent) => {
    if (node.type.name === 'paragraph' && node.childCount > 0) {
      ...
      if (lastChild && lastChild.attrs.isCustomSlot) {
        ...
        if (prevChar !== _constants.strings.ZERO_WIDTH_CHAR) {
          todoPositions.push(paragraphEndPos);
        }
      }
      if (lastChild === firstChild && lastChild.isText && lastChild.text === _constants.strings.ZERO_WIDTH_CHAR) {
  ...
  if (todoPositions.length > 0) {
    ...
    todoPositions.sort((a, b) => {
      ...
      if (Array.isArray(insertPos) && insertPos[0] === 'remove') {
        ...
        tr = tr.insertText(_constants.strings.ZERO_WIDTH_CHAR, insertPos, insertPos);
      }
    });
    return tr;
  }
...
function ensureTrailingText(schema) {
  return new _prosemirrorState.Plugin({
    appendTransaction(transactions, oldState, newState) {
      if (transactions.some(tr => tr.getMeta(_constants.strings.DELETABLE))) {
...
function keyDownHandlePlugin(schema) {
  return new _prosemirrorState.Plugin({
    key: new _prosemirrorState.PluginKey('prevent-empty-inline-node'),
    props: {
      handleKeyDown(view, event) {
        ...
        if (event.key === 'ArrowLeft' && node.type.name !== 'inputSlot') {
          if ($from.nodeBefore && $from.nodeBefore.isText && $from.nodeBefore.text) {
            if ($from.nodeBefore.text === _constants.strings.ZERO_WIDTH_CHAR) {
              // 获取零宽字符前的节点
              ...
              if (index >= 2) {
                ...
                if (secondBeforeCursorNode.attrs.isCustomSlot) {
                  ...
                  dispatch(state.tr.setSelection(_prosemirrorState.TextSelection.create(state.doc, nextCursorPos)));
                ...
                nextCursorPos > 0 && dispatch(state.tr.setSelection(_prosemirrorState.TextSelection.create(state.doc, nextCursorPos)));
                event.preventDefault();
                return true;
              }
            } else if ($from.nodeBefore.text.endsWith(_constants.strings.ZERO_WIDTH_CHAR)) {
        ...
        if (event.key === 'ArrowRight' && node.type.name !== 'inputSlot') {
          if ($from.nodeAfter && $from.nodeAfter.isText) {
            if ($from.nodeAfter.text === _constants.strings.ZERO_WIDTH_CHAR) {
              /**
              ...
              if (index < parent.children.length - 1) {
                ...
                if (secondAfterCursorNode.attrs.isCustomSlot) {
                  ...
                  dispatch(state.tr.setSelection(_prosemirrorState.TextSelection.create(state.doc, newPos)));
                  event.preventDefault();
                  return true;
                }
            ...
            } else if ($from.nodeBefore && $from.nodeBefore.isText && $from.nodeBefore.text.startsWith(_constants.strings.ZERO_WIDTH_CHAR)) {
              // Backup，当零宽字符出现在 text 节点中
              const nextCursorPos = $from.pos + 2;
              dispatch(state.tr.setSelection(_prosemirrorState.TextSelection.create(state.doc, nextCursorPos)));
              event.preventDefault();
        ...
        if (event.key === 'Backspace' && selection.empty) {
           ...
           */
          if ($from.nodeBefore && $from.nodeBefore.isText && ((_a = $from.nodeBefore.text) === null || _a === void 0 ? void 0 : _a.length) === 1 && $from.nodeBefore.text !== _constants.strings.ZERO_WIDTH_CHAR && $from.nodeAfter && $from.nodeAfter.attrs.isCustomSlot) {
            const begin = $from.pos - $from.nodeBefore.nodeSize;
            const end = $from.pos;
            let tr = state.tr.delete(begin, end);
            tr = tr.insertText(_constants.strings.ZERO_WIDTH_CHAR, begin, begin);
            dispatch(tr);
            event.preventDefault();
            return true;
          }
          ...
          if (beforeNode && beforeNode.attrs.isCustomSlot) {
            ...
            if (index > 1) {
              ...
              if (prevPrevNode && prevPrevNode.isText && prevPrevNode.text.endsWith(_constants.strings.ZERO_WIDTH_CHAR)) {
                deleteStart = deleteStart - 1;
              }
            }
            if (afterNode.isText && afterNode.text.startsWith(_constants.strings.ZERO_WIDTH_CHAR)) {
          ...
          if (afterNode && afterNode.isText && afterNode.text === _constants.strings.ZERO_WIDTH_CHAR) {
            const index = $from.index(); // 当前光标在 parent.children 中的 offset
          ...
          if (beforeNode && beforeNode.isText && beforeNode.text === _constants.strings.ZERO_WIDTH_CHAR) {
            const parent = $from.parent;
            const index = $from.index(); // 当前光标在 parent.children 中的 offset
            if (index > 1) {
            ...
            if (node.type.name === 'inputSlot' && $from.pos === $from.start()) {
              ...
              if (parentIndex > 0) {
                ...
                if (parentPrevNode && parentPrevNode.isText && parentPrevNode.text.endsWith(_constants.strings.ZERO_WIDTH_CHAR)) {
                  const pos = $from.pos - 2;
                  dispatch(state.tr.setSelection(_prosemirrorState.TextSelection.create(state.doc, pos)));
                  event.preventDefault();
                  return true;
                }
              }
              ...
              let tr = state.tr.insertText(_constants.strings.ZERO_WIDTH_CHAR, pos, pos + 1);
              tr = tr.setSelection(_prosemirrorState.TextSelection.create(tr.doc, pos));
              dispatch(tr);
              event.preventDefault();
              return true;
            }
        ...
        if (event.key === 'Backspace' && !selection.empty) {
          ...
          if (nodeBefore && nodeBefore.isText && nodeBefore.text.endsWith(_constants.strings.ZERO_WIDTH_CHAR)) {
            startPos -= 1;
          }
          if (nodeAfter && nodeAfter.isText && nodeAfter.text.startsWith(_constants.strings.ZERO_WIDTH_CHAR)) {
            endPos += 1;
          }
        ...
        if (node.type.name === 'inputSlot') {
          ...
          if (node.textContent === _constants.strings.ZERO_WIDTH_CHAR && (event.key === 'ArrowLeft' || event.key === 'ArrowRight')) {
          ...
          if ($from.pos === $from.end() && node.textContent.length === 1 && node.textContent !== _constants.strings.ZERO_WIDTH_CHAR && event.key === 'Backspace') {
            const pos = $from.pos - 1;
            dispatch(state.tr.insertText(_constants.strings.ZERO_WIDTH_CHAR, pos, pos + 1));
            event.preventDefault();
            return true;
          }
          // 全选 input-slot 节点内容时，点击删除，插入零宽字符
          // When selecting all input-slot node content, insert zero-width characters
          if (!selection.empty && $from.parent === node && selection.from === $from.start() && selection.to >= $from.end() && event.key === 'Backspace') {
            const tr = state.tr;
            ...
            tr.insertText(_constants.strings.ZERO_WIDTH_CHAR, $from.start(), $from.end());
            const pos = $from.start() + 1; // 1 是零宽字符的长度
            tr.setSelection(_prosemirrorState.TextSelection.create(tr.doc, pos));
            dispatch(tr);
            event.preventDefault();
            return true;
          }
          ...
          if (node.textContent === _constants.strings.ZERO_WIDTH_CHAR && event.key === 'Backspace') {
            // 计算当前节点在文档中的位置
...
function handlePasteLogic(view, event) {
  ...
  tr.setMeta(_constants.strings.DELETABLE, true);
  dispatch(tr);
  return false;
}
function removeZeroWidthChar($from, tr) {
  ...
  if ($from.nodeBefore && $from.nodeBefore.isText && $from.nodeBefore.text === _constants.strings.ZERO_WIDTH_CHAR) {
    tr = tr.delete($from.pos - $from.nodeBefore.nodeSize, $from.pos);
    return true;
  }
  // Check the node after the cursor
  if ($from.nodeAfter && $from.nodeAfter.isText && $from.nodeAfter.text === _constants.strings.ZERO_WIDTH_CHAR) {
...
function removeZeroWidthCharForComposition($from, tr) {
  ...
  if ($from.nodeBefore && $from.nodeBefore.isText) {
    ...
    if (text === null || text === void 0 ? void 0 : text.startsWith(_constants.strings.ZERO_WIDTH_CHAR)) {
      // 删除第一个字符
